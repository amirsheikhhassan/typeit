...
function TypeIt(element, options) {
  ...
  this.delete = function (numCharacters, actionOpts) {
    ...
    return _queueAndReturn([bookEndQueueItems[0]].concat(_toConsumableArray(_toConsumableArray(Array(Math.abs(numCharacters) || 1)).fill().map(() => [_delete, !numCharacters, _freezeCursorMeta])), [bookEndQueueItems[1]]), 1, actionOpts);
  };
  ...
  this.move = function (movementArg, actionOpts) {
    ...
    return _queueAndReturn([bookEndQueueItems[0]].concat(_toConsumableArray(_toConsumableArray(Array(Math.abs(movementArg) || 1)).fill().map(() => [_move, moveArg, _freezeCursorMeta])), [bookEndQueueItems[1]]), 1, actionOpts);
  };
