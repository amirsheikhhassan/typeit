(function (global, factory) {
  ...
  (global = global || self, global.TypeIt = factory());
  ...
  * @return {array}
  */
  var guaranteeThreeKeys = (function (q) {
    return q.map(function (queueItem) {
  ...
  var merge = (function (originalObj, newObj) {
  ...
  var setPropToMeta = function setPropToMeta(item, toMerge) {
    item[2] = merge(item[2], toMerge) || toMerge;
    return item;
  };

  var setMetaProperty = (function (queueItems, toMerge) {
    if (!isArray(queueItems[0])) {
      return setPropToMeta(queueItems, toMerge);
    }

    return queueItems.map(function (i) {
  ...
  var queueMany = (function (arrayOrNumber, actionToQueue, metaObject, shouldBookEnd) {
    ...
    return arrayOrNumber.map(function (item, index) {
      if (isNumber) {
  ...
  function Queue(initialItem) {
    var _setIdOnItems = function _setIdOnItems(items) {
      return guaranteeThreeKeys(items).map(function (step) {
        step[2] = merge(step[2], {
          id: _id
        });
        _id++;
    ...
    this.reset = function () {
      _queue = _queue.map(function (item) {
    ...
    this.getItems = function () {
      ...
      return _queue.filter(function (i) {
        return !i[2].executed;
      });
    };
    /**
    ...
    this.setMeta = function (id, meta) {
      var index = _queue.findIndex(function (i) {
        return i[2].id === id;
      });
   ...
   * @return {object}
   */
  var getParsedBody = (function (content) {
   ...
   * @return {boolean}
   */
  var isTypeableNode = (function (node) {
  ...
  var getAllTypeableNodes = function getAllTypeableNodes(element, parentToExclude, shouldReverse) {
    parentToExclude = parentToExclude || null;
    shouldReverse = shouldReverse !== undefined ? shouldReverse : false;
    var nodes = toArray(element.childNodes).map(function (child) {
    ...
    if (parentToExclude) {
      nodes = nodes.filter(function (n) {
        return !parentToExclude.contains(n);
      });
    }
  ...
  var constructQueueFromNodes = function constructQueueFromNodes(el) {
    var nodeList = getAllTypeableNodes(el);
    var characterObjects = nodeList.map(function (item) {
      if (!item.nodeValue) {
      ...
      return stringToArray(item.nodeValue).map(function (character) {
  ...
  var createCharacterObject = function createCharacterObject(content, node) {
    node = node || null;
  ...
  function maybeChunkStringAsHtml(str, asHtml) {
    ...
    return toArray(str).map(function (char) {
  ...
  var appendStyleBlock = (function (styles, id) {
  ...
  var calculateDelay = (function (delayArg) {
    if (!isArray(delayArg)) {
  ...
  var destroyTimeouts = (function (timeouts) {
  ...
  var fireWhenVisible = (function (element, func) {
  ...
  var findPrintedNode = function findPrintedNode(node, element) {
    var printedNodes = element.querySelectorAll("*");
    return [element].concat(toArray(printedNodes).reverse()).find(function (i) {
  ...
  var isLastElement = function isLastElement(node, nodeToIgnore) {
    if (!node) {
      return false;
    }
  ...
  var insertIntoElement = (function (element, contentArg, cursorNode, cursorPosition) {
  ...
  var removeEmptyElements = (function (node) {
    toArray(node.querySelectorAll("*")).forEach(function (i) {
      if (!i.innerHTML && i.tagName !== "BR") {
        var nodeToRemove = i;

        while (nodeToRemove.parentNode.childNodes.length === 1 && nodeToRemove.parentNode.childNodes[0].isEqualNode(nodeToRemove)) {
  ...
  var repositionCursor = (function (element, allChars, cursor, cursorPosition) {
    if (!cursor) {
      return;
    }
  ...
  var processCursorMovementArg = (function (movementArg, currentCursorPosition, allChars) {
  ...
  var getStyleString = (function (fromElement) {
  ...
  function TypeIt(element, options) {
    ...
    var _maybeAppendPause = function _maybeAppendPause(opts) {
      opts = opts || {};
      var delay = opts.delay;
      delay && _queue.add([_pause, delay]);
    };

    var _queueAndReturn = function _queueAndReturn(stepOrSteps, numberOfTimes, opts) {
    ...
    var _generateTemporaryOptionQueueItems = function _generateTemporaryOptionQueueItems(newOptions) {
      newOptions = _typeof(newOptions) === "object" ? newOptions : {};
    ...
    var _addSplitPause = function _addSplitPause(startPosition, numberOfActionsToWrap) {
    ...
    var _maybePrependHardcodedStrings = function _maybePrependHardcodedStrings(strings) {
      ...
      if (_opts.startDelete) {
        chunkStringAsHtml(existingMarkup).forEach(function (item) {
    ...
    var _pause = function _pause(time) {
    ...
    var _move = function _move(movementArg) {
    ...
    var _type = function _type(characterObject) {
      return new Promise(function (resolve) {
        _wait(function () {
    ...
    var _empty = _async(function () {
      ...
      _getAllChars().forEach(function (n) {
        removeNode(n);
      });
    });

    var _delete = function _delete(keepGoingUntilAllIsGone) {
    ...
    this.delete = function (numCharacters, actionOpts) {
      ...
      return _queueAndReturn([bookEndQueueItems[0]].concat(_toConsumableArray(_toConsumableArray(Array(Math.abs(numCharacters) || 1)).fill().map(function () {
        return [_delete, !numCharacters, _freezeCursorMeta];
      })), [bookEndQueueItems[1]]), 1, actionOpts);
    };
    ...
    this.move = function (movementArg, actionOpts) {
      ...
      return _queueAndReturn([bookEndQueueItems[0]].concat(_toConsumableArray(_toConsumableArray(Array(Math.abs(movementArg) || 1)).fill().map(function () {
        return [_move, moveArg, _freezeCursorMeta];
      })), [bookEndQueueItems[1]]), 1, actionOpts);
    };
    ...
    this.type = function (string, actionOpts) {
      ...
      var chunkedString = maybeChunkStringAsHtml(string, _opts.html);
      var itemsToQueue = [bookEndQueueItems[0]].concat(_toConsumableArray(queueMany(chunkedString, _type, _freezeCursorMeta, true)), [bookEndQueueItems[1]]);
